...
export default function EmployeeDailyPage() {
  ...
  const groups: Group[] = useMemo(() => {
    ...
    for (const g of arr) {
      g.items.sort((a, b) => normLabel(a.sub_label).localeCompare(normLabel(b.sub_label)));
    }
  ...
  return (
    <div style={{ padding: 24, maxWidth: 980 }}>
      ...
      {status ? <div style={{ marginTop: 14, fontSize: 12, opacity: 0.9 }}>{status}</div> : null}
      ...
      <div style={{ marginTop: 10, display: "grid", gap: 14 }}>
        {groups.map((g) => (
          ...
          >
            ...
            <div style={{ fontSize: 12, opacity: 0.75, marginTop: 2 }}>Unit: {g.unit}</div>
              ...
              {g.items.map((ti) => (
                ...
                >
                  ...
                  >
                    ...
                    <button onClick={() => saveEntry(ti.id)} style={{ padding: "10px 14px", minWidth: 120 }}>
                      Save
                    </button>
                  </div>

                  {lastSaved[ti.id] ? (
                    <div style={{ marginTop: 8, fontSize: 12, opacity: 0.8 }}>{lastSaved[ti.id]}</div>
                  ) : null}
                </div>
              ))}
            </div>
